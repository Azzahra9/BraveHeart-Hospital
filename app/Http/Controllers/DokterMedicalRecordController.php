...
class DokterMedicalRecordController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (!$appointmentId) {
            return redirect()->route('dokter.appointments.index')->with('error', 'ID Janji Temu tidak ditemukan.');
        }

        // Ambil data appointment dan pastikan milik dokter ini
        $appointment = Appointment::with('pasien')->where('id', $appointmentId)->where('dokter_id', Auth::id())->firstOrFail();

        // Ambil daftar obat untuk resep
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('dokter.appointments.index')->with('success', 'Pemeriksaan selesai! Rekam medis berhasil disimpan.');
    }
}